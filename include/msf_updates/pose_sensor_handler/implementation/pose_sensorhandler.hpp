...
//  ros::NodeHandle pnh("~/" + parameternamespace);
  ...
  provides_absolute_measurements_ = Config::get<bool>(parameternamespace + "/pose_absolute_measurements", true);
  measurement_world_sensor_ = Config::get<bool>(parameternamespace + "/pose_measurement_world_sensor", true);
  use_fixed_covariance_ = Config::get<bool>(parameternamespace + "/pose_use_fixed_covariance", false);
  pose_measurement_minimum_dt_ = Config::get<double>(parameternamespace + "/pose_measurement_minimum_dt", 0.05);
  enable_mah_outlier_rejection_ = Config::get<bool>(parameternamespace + "/enable_mah_outlier_rejection", false);
  mah_threshold_ = Config::get<double>(parameternamespace + "/mah_threshold", msf_core::kDefaultMahThreshold_);
...
//    double distortpos_mean = Config::get<double>(parameternamespace + "/distortpos_mean", 0.0);
////    pnh.param("distortpos_mean", distortpos_mean, 0.0);
//    meanpos.setConstant(distortpos_mean);
//
//    Eigen::Vector3d stddevpos;
//    double distortpos_stddev = Config::get<double>(parameternamespace + "/distortpos_stddev", 0.0);
////    pnh.param("distortpos_stddev", distortpos_stddev, 0.0);
//    stddevpos.setConstant(distortpos_stddev);
//
//    Eigen::Vector3d meanatt;
//    double distortatt_mean = Config::get<double>(parameternamespace + "/distortatt_mean", 0.0);
////    pnh.param("distortatt_mean", distortatt_mean, 0.0);
//    meanatt.setConstant(distortatt_mean);
//
//    Eigen::Vector3d stddevatt;
//    double distortatt_stddev = Config::get<double>(parameternamespace + "/distortatt_stddev", 0.0);
////    pnh.param("distortatt_stddev", distortatt_stddev, 0.0);
//    stddevatt.setConstant(distortatt_stddev);
//
//    double distortscale_mean = Config::get<double>(parameternamespace + "/distortscale_mean", 0.0);
////    pnh.param("distortscale_mean", distortscale_mean, 0.0);
//    double distortscale_stddev = Config::get<double>(parameternamespace + "/distortscale_stddev", 0.0);
...
void PoseSensorHandler<MEASUREMENT_TYPE, MANAGER_TYPE>::ProcessPoseMeasurement(
  ...
  if (mngr) {
    bool pose_fixed_scale = Config::get<bool>(parameternamespace_ + "/pose_fixed_scale", true);
    if (pose_fixed_scale) {
      fixedstates |= 1 << MEASUREMENT_TYPE::AuxState::L;
    }
    bool pose_fixed_p_ic = Config::get<bool>(parameternamespace_ + "/pose_fixed_p_ic", true);
    if (pose_fixed_p_ic) {
      fixedstates |= 1 << MEASUREMENT_TYPE::AuxState::p_ic;
    }
    bool pose_fixed_q_ic = Config::get<bool>(parameternamespace_ + "/pose_fixed_q_ic", true);
    if (pose_fixed_q_ic) {
      fixedstates |= 1 << MEASUREMENT_TYPE::AuxState::q_ic;
    }
    bool pose_fixed_p_wv = Config::get<bool>(parameternamespace_ + "/pose_fixed_p_wv", true);
    if (pose_fixed_p_wv) {
      fixedstates |= 1 << MEASUREMENT_TYPE::AuxState::p_wv;
    }
    bool pose_fixed_q_wv = Config::get<bool>(parameternamespace_ + "pose_fixed_q_wv", true);
    if (pose_fixed_q_wv) {
...
//  this->SequenceWatchDog(msg->header.seq, "TransformStamped");
//  MSF_INFO_STREAM_ONCE(
//      "*** pose sensor got first measurement from topic "
//          << this->topic_namespace_ << "/" << "TransformStamped"
//          << " ***");
//
//  double time_now = msg->header.stamp.toSec();
//  const double epsilon = 0.001; // Small time correction to avoid rounding errors in the timestamps.
...
//  this->SequenceWatchDog(msg->header.seq, "PoseStamped");
//  MSF_INFO_STREAM_ONCE(
//      "*** pose sensor got first measurement from topic "
//          << this->topic_namespace_ << "/" << "PoseStamped"
//          << " ***");
//
//  geometry_msgs::PoseWithCovarianceStampedPtr pose(
//      new geometry_msgs::PoseWithCovarianceStamped());
